...
public class CartService {
    ...
    public void addToCart(Integer id,
        ...
        if (existingItem.isPresent()) {
            ...
            cartItem.setPrice(Math.round((price * quantity) * 100) / 100d);
    ...
    public void updateCartItemQuantity(Integer id, Integer productId, int quantity, double newPrice) {
        Cart cart = cartRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Shopping cart not found"));
    ...
    public void increaseQuantity(Integer id, Integer productId) {
        ...
        for (CartItem item : items) {
            if (item.getProduct().getProductId().equals(productId)) {
                ...
                item.setPrice(Math.round((item.getPrice() + item.getProduct().getPrice()) * 100) / 100d);
                break;
            }
        }
    ...
    public double calculateTotalPrice(Integer id) {
        ...
        for (CartItem item : cart.getItems()) {
            double totalPricePerItem = item.getProduct().getPrice() * item.getQuantity();
